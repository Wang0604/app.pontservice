...
- ¥999 咨询服务 → 中小企业客单价
- ¥2999 OCR 工具包 → 中小企业客单价
- ¥36000 年付（打包价） → 中型企业客单价
...
│ ───────                                                         │
│ 浏览: /pricing 三档套餐展示                                       │
│   • ¥999 - 单次咨询（对应 1 次 AI 审计 + 1 小时远程咨询）           │
│   • ¥2999 - OCR 工具包（12 个月使用权 + 200 credits/月）           │
│   • ¥36000 - 年度全包（12 个月全工具 + 2400 credits 一次到账）     │
│   • 早鸟价: 前 10 位客户享 2999 → 1999 / 36000 → 24000            │
│ CTA: "立即申请" 按钮                                              │
└────────────────────────────────────────────────────────────────┘
                              │
                              ▼
...
create table leads (
  ...
  email text not null,
  phone text,
  interested_plan text, -- '999' | '2999' | '36000' | 'combo'
  use_case text,
  notes text,
...
create table orders (
  ...
  plan_type text not null, -- '999' | '2999' | '36000'
...
关键条款:
- 服务范围: OCR 工具包 12 个月使用权，200 credits/月（年度累计 2400 credits）
- 服务期限: 付款激活之日起 12 个月
...
关键条款:
- 服务范围: 全工具矩阵 12 个月访问权，2400 credits 一次性到账
- 早鸟价: 注明原价 36000，早鸟价 24000（按实际谈判填写）
- 退款条款: 激活 14 天内未使用可全退，激活 14 天后按月退款
- 优先级: 提供优先客服响应（工作日 4 小时内）
- 个性化: 可申请 2 次定制化 prompt 调优
...
你提到"按年交的话可以给他们一些优惠"。建议这样设计:

| 套餐 | 原价 | 年付实付 | 折扣 | 客户心智 |
|------|------|---------|-----|---------|
| 2999 OCR 工具包 | 2999×12 = 35988/年 | 24000 | 33% 折扣 | "省 12000" |
| 单买 36000 | 36000/年 | 36000 | 无折扣 | 全包 |
| 早鸟年付 | 36000 | 19999 | 45% 折扣 | "前 10 名" |
| 早鸟月付 | 2999 | 1999 | 33% 折扣 | "前 10 名" |

**关键**:
- 早鸟价是"前 10 名限定"，制造稀缺感
- 过了前 10 名恢复原价，老客户续费享老价格
- 年付一次性到账 credits 避免月月提醒

**合同里的文字**:
```
"甲方作为本服务前 10 位签约客户，享受早鸟优惠价: 
 原价¥36,000元 → 早鸟价¥24,000元。
 本价格仅限本合同期内有效，续约恢复届时公开价格（以年为单位可再次享受老客户续费优惠）。"
```
...
|------|---------|------|
| 999 咨询 | 50 | 够试用 + 生成报告 |
| 2999 月/包 | 200/月 或 2400/年 | 按合同约定 |
| 36000 年付 | 2400 一次性 | 当年内用完 |
| 早鸟 1999 | 同 2999 | 数量不变，只是价格优惠 |
| 早鸟 24000 | 同 36000 | 数量不变，只是价格优惠 |
| 注册送 | 50 | 不依赖付费 |

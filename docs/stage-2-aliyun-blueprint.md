# Stage 2 蓝图: 阿里云 Supabase 全家桶方案

> **版本**: 2.0 (阿里云 Supabase 全家桶方案)
> **归档日期**: 2026-04-24
> **状态**: 作为 Stage 2 国内规模化时的参考蓝图保留
> **当前决策**: 不执行（详见 [ADR-002](adr-002-global-first-mvp.md)）
> **执行条件**: [stage-plan.md](stage-plan.md) §Stage 2 触发条件满足后

本文档是 2026-04-23 起草的 v2.0 架构方案，基于"国内 B 端合规"前提。2026-04-24 客户画像澄清为"海外优先 + 国内点状大客户"后，本方案暂缓执行，转向 [README.md](../README.md) 的 v3.0 Railway + Cloudflare + Inngest Stage 0 方案。

本蓝图在 Stage 2 触发时恢复执行（需增加 ICP 备案 + 微信支付商户号 + 阿里云企业认证等前置条件）。
...
## 五、Feature 路线图 (M0-M5，原 v2.0 节奏)

> 注: 此节奏为 Stage 2 启动后的规模化节奏，Stage 0/1 采用 [roadmap.md](roadmap.md) 的新节奏。
...
## 八、前置检查清单 (Stage 2 启动前必须完成)

| # | 事项 | 预计耗时 |
|---|------|---------|
| 1 | ICP 备案提交 | 20 工作日 |
| 2 | 微信支付商户号开户 | 7-15 工作日 |
| 3 | 阿里云账号企业认证 | 1-3 工作日 |
| 4 | 域名备案确认 (`pontai.com` 或 `.cn`) | - |
| 5 | 阿里云短信模板审批 | 1-3 工作日 |
| 6 | 支付宝开放平台认证 | 3-7 工作日 |
...
> **Next.js + 阿里云 SAE + 阿里云 AnalyticDB Supabase + 阿里云 CDN + 微信支付 + DeepSeek/火山方舟，全栈阿里云生态，数据全在中国境内，月单 30-1000 这套架构无需改动，单月成本 ~¥500-800。**

---

## 十、相关文档

- [README.md](../README.md) — 当前生效的 v3.0 Railway + Cloudflare Stage 0 架构
- [ADR-001: 选择阿里云 Supabase 全家桶](adr-001-supabase-alibaba.md) — 本方案的决策依据
- [ADR-002: 转向海外优先 MVP](adr-002-global-first-mvp.md) — 覆盖本方案的决策
- [stage-plan.md](stage-plan.md) — Stage 2 触发条件
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 1.0 | 2026-04-23 | 初版（Vercel + Supabase）|
| 1.1 | 2026-04-23 | 部署平台从 Vercel 切换为 Railway |
| 2.0 | 2026-04-23 | 全盘切换为阿里云 Supabase 全家桶 |
| **归档** | **2026-04-24** | **根据 ADR-002 归档，作为 Stage 2 蓝图保留** |
